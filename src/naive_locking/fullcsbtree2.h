...
class FullCSBTree : public CSBTree
{
    private:
#ifdef NAIVE_LOCKING
        pthread_mutex_t tree_access_lock;
#endif
    ...
    public:
...
#ifdef NAIVE_LOCKING
                    // Init mutex
                    if(pthread_mutex_init(&tree_access_lock, NULL) != 0) {
                        cout<<"Tree mutex init failed"<<endl;
                        exit(1);
                    }
#endif
		}
		...
		{
#ifdef NAIVE_LOCKING
                    pthread_mutex_lock(&tree_access_lock);
#endif
...
#ifdef NAIVE_LOCKING
                    pthread_mutex_unlock(&tree_access_lock);
#endif
		    return 0;
		}
